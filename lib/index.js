...
export default class PmpScraper extends EventEmitter {
  ...
  scrapePage(args, done) {
    ...
    schema.validate(args, (err, val) => {
      ...
      async.auto({
        ...
        this.emit('info', 'report-' + val.pageNumber, {
          report: results.generateReport.report
        });

        if(val.pageNumber % this.options.statsInterval == 0) {
          this.emit('info', 'stats-' + val.pageNumber, {
  ...
  scrapePageForever(args, done) {
    ...
    schema.validate(args, (err, val) => {
      ...
      this.emit('info', 'scrapeSource.start', {
      ...
      async.forever(next => {
        ...
        this.emit('error', 'async.forever', err);

        if (_.isString(thresholdMessage)) {
          this.emit('info', 'scrapeSource.end', {
            source: val.source.id,
            pageNumber: this.pageNumber,
            stats: this.stats.getStats(true)
          });
  ...
  reindexImages() {
    this.emit('warn', 'reindexImages', 'started');

    main.reindexImages({
      options: this.options
    }, (err, res) => {
      if (err) {
        this.emit('error', 'reindexImages', err);
        return;
      }

      this.emit('info', 'reindexImages', res);
    });
  }
